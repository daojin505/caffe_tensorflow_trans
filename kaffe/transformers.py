...
class DataInjector(object):
    ...
    def load_using_caffe(self):
        ...
        #W = net.params['conv1_bn'][0].data[...]
        #b = net.params['conv1_bn'][1].data[...]
        ...
        #    print(len(v))
        for k, v in net.params.items():
            param_data = list(map(data, v))
            self.params.append((k, param_data))
    ...
    def __call__(self, graph):
        for layer_name, data in self.params:
            if layer_name in graph:
                #print("data injecter node name " +  layer_name)
                #print("data injecter node size " + str(len(list(data))))
                node = graph.get_node(layer_name)
                node.data = self.adjust_parameters(node, data)
                #print("data injecter node after size " + str(len(list(node.data))))
            else:
...
class BatchNormPreprocessor(object):
    ...
    def __call__(self, graph):
        ...
        for node in graph.nodes:
            ...
            print(node.name)
            print(len(list(node.data)))

            #assert len(list(node.data)) == 3
